...
void GenMipmapsApp::GenerateMipmaps (uint mode)
{
	...
	vkCmdPipelineBarrier( cmd, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, null, 0, null, 0, null );
	...
	switch ( mode )
	{
		case 0 :	GenMipmapsOnComputeShader( cmd, OUT curr_layout );		name = "using compute shader";		break;
		case 1 :	GenMipmapsOnGraphicsShader( cmd, OUT curr_layout );		name = "using render pass";			break;
		case 2 :	GenMipmapsOnTransferCommands( cmd, OUT curr_layout );	name = "using image blit";			break;
		case 3 :	GenMipmapsOnTransferCommands2( cmd, OUT curr_layout );	name = "using optimal image blit";	break;
...
bool GenMipmapsApp::CreateDownscaleComputePipeline ()
{
	...
	uint2	local_size;
	{
		const uint	max_invocations = vulkan.GetProperties().properties.limits.maxComputeWorkGroupInvocations;
		uint2		size {2};
